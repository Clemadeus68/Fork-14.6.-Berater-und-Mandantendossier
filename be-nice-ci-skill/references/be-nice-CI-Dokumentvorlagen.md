...
|---|---|---|
| `be-nice Kopie.png` | Header oben links (echte Transparenz — RGBA) | aus Datei lesen (aktuell 2493 × 887 px ≈ 2.81 : 1) |
...
```javascript
// Linke Zelle: be nice Logo — Maße IMMER aus der Datei berechnen, nie hartkodieren
const LOGO_PX_W  = IMG_BENICE.readUInt32BE(16);  // PNG-IHDR: Breite
const LOGO_PX_H  = IMG_BENICE.readUInt32BE(20);  // PNG-IHDR: Höhe
const HDR_LOGO_W = 125;
const HDR_LOGO_H = Math.round(HDR_LOGO_W * LOGO_PX_H / LOGO_PX_W);
...
be nice Logo links: Höhe `HEADER_H - Cm(0.2)`, Breite = Höhe × Seitenverhältnis aus der Datei (`img_ratio(LOGO_BENICE)` im Template), x = `Cm(0.4)`, y = `Cm(0.1)`.  
